...
<<<<<<< ours
=======
const API_BASE = window.__API_BASE__ || (window.location.port === '3000' ? '' : 'http://localhost:3000');
>>>>>>> theirs
...
function updateServerStatus(state, message) {
  ...
  const normalized = state === 'online' ? 'online' : 'offline';
  serverStatusEl.classList.remove('online', 'offline');
  serverStatusEl.classList.add(normalized);
  serverStatusEl.textContent = message || (normalized === 'online' ? 'Server: online' : 'Server: offline');
}

async function checkServerConnectivity({ silent = false } = {}) {
  if (!serverStatusEl) return false;
  try {
    ...
    updateServerStatus('online', `Server: online (${ts})`);
    return true;
  } catch (error) {
    if (!silent) {
      console.warn('[Client] Health check failed:', error);
    }
    updateServerStatus('offline', 'Server: offline (health check failed)');
    return false;
  }
}

checkServerConnectivity();
setInterval(() => checkServerConnectivity({ silent: true }), 60000);
...
<<<<<<< ours
=======
    updateServerStatus('offline', 'Server: offline (request failed, using fallback)');
>>>>>>> theirs
...
=======
  console.log('[Client] submitting round', {
    round: gameState.round,
    textLength: text.length,
    apiKeyProvided: Boolean(userApiKey)
  });
  const response = await fetch(`${API_BASE}/api/process-round`, {
>>>>>>> theirs
    method: 'POST',
...
=======
  ...
  updateServerStatus('online', `Server: online (last round ${new Date().toLocaleTimeString()})`);
>>>>>>> theirs
